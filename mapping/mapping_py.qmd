...
# Legend using patches for the Jenks intervals — place it outside the axes on the right so it won't overlap the scale bar
patches = [Patch(facecolor=colors[i], edgecolor='black', label=labels[i]) for i in range(len(labels))]

# make room on the right for the legend, then place legend outside the axes
fig = ax.get_figure()
fig.subplots_adjust(right=0.78)  # leave space on the right for the legend
ax.legend(
  handles=patches,
...
# draw scale bar at lower right
scale_bar(ax, location="lower right", style="boxes", bar={"projection": census_bpop_2020.crs, "length": 0.18})
